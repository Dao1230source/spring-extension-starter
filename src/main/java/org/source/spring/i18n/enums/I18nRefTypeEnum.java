...
public enum I18nRefTypeEnum {
    ...
    public static @Nullable String getI18nDictGroup(I18nDict i18nDict, Class<? extends Enum<?>> enumClass, boolean single) {
        String groupName = getName(i18nDict.group(), enumClass);
        if (single) {
            return groupName;
        }
        String keyName = getName(i18nDict.key(), enumClass);
        if (StringUtils.hasText(keyName)) {
            groupName += Constants.COLON + keyName;

        }
        String valueName = getName(i18nDict.value(), enumClass);
