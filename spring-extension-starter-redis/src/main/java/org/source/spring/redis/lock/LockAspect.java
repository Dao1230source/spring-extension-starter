...
public class LockAspect {
    ...
    public Object inventoryLock(ProceedingJoinPoint point, Lock lock) throws Throwable {
        String name = point.getSignature().toShortString();
        log.debug("为该方法[{}]添加分布式锁, distributedLock:{}", name, lock);
        RLock rLock = getRedissonLock(point, lock);
    ...
    public RLock getRedissonLock(ProceedingJoinPoint point, Lock lock) {
        String key = lock.key();
        ...
        Object value;
        Lock.ValueParser valueParser = lock.valueParser();
